...
export function AuthorForm({own,state,controller,user,profile,businessId,heading=false}) {
  ...
  return <><div className="customer-reviews__heading">{heading?<h2 id="customer-reviews-title" tabIndex={-1} ref={title}>{t('customerReviews.title')}</h2>:<span tabIndex={-1} ref={title} aria-label={t('customerReviews.myReview')}/>}
    ...
    <label>{t('customerReviews.text')}<textarea required disabled={blocked} value={text} onChange={e=>setDraft({rating,text:e.target.value,displayName})} aria-invalid={invalid||undefined} aria-describedby={`${id}-count ${id}-translation-notice${invalid?` ${id}-error`: ''}`}/></label>
    <p id={`${id}-translation-notice`} className="customer-reviews__notice">{t('customerReviews.translationNotice')} <Link to="/privacy">{t('customerReviews.privacyLink')}</Link></p>
