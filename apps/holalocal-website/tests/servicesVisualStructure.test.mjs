...
test('Services results stay compact without an introduction wash', async () => {
  ...
  const directory = await readFile(new URL('../src/styles/servicesPresentation.css', import.meta.url), 'utf8')
  assert.match(directory, /\.directory-business-card \{/)
  assert.match(directory, /-webkit-line-clamp: 2/)
  assert.doesNotMatch(styles, /public-business-card__result|public-business-card--result/)
})
