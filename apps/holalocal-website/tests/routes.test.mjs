...
test('homepage hero fills the viewport below the responsive header without clipping content', async () => {
  ...
  assert.match(styles, /\.marketing-hero\s*\{[\s\S]*?width: 100%;[\s\S]*?min-height: calc\(100vh - var\(--site-header-height\)\);[\s\S]*?min-height: calc\(100svh - var\(--site-header-height\)\);[\s\S]*?margin: 0;/)
  assert.doesNotMatch(styles.match(/\.marketing-hero\s*\{[^}]*\}/)?.[0] ?? '', /100dvh/)
