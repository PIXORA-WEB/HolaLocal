...
const server=await createServer({root,configFile:false,envDir:false,server:{host:'127.0.0.1',port:0},plugins:[{
 ...
 window.calls=0;window.pending=[];window.behavior='success';const answer=p=>({...p,status:'translated',translatedText:'translated-'+p.targetLanguage+'-r'+p.publishedRevision+' <img src=x onerror=alert(1)>'});
 const api={translate:async p=>{window.calls++;if(window.behavior==='fail')throw new Error('private upstream diagnostic');if(window.behavior==='defer')return new Promise(resolve=>window.pending.push({language:p.targetLanguage,revision:p.publishedRevision,resolve:()=>resolve(answer(p))}));return answer(p)}};
 function App(){const [source,setSource]=useState(null);const [revision,setRevision]=useState(1);window.control={source:setSource,language:l=>i18n.changeLanguage(l),revision:setRevision};return React.createElement('article',null,React.createElement('h3',null,'Unchanged public name'),React.createElement(ReviewText,{api,review:{publicReviewId:'synthetic-public-review',publishedRevision:revision,originalText:'Original synthetic review r'+revision,declaredSourceLanguage:source}}))};createRoot(document.getElementById('root')).render(React.createElement(App));`
...
try{
 for(const width of [390,1440]){
  ...
  await page.evaluate(()=>window.control.source('de'));await page.waitForFunction(()=>document.querySelector('[role=status]').textContent==='')
  assert.equal(await page.evaluate(()=>window.calls),before);assert.equal(await page.locator('.customer-reviews__text').textContent(),'Original synthetic review r2');assert.equal(await page.locator('button').count(),0)
