...
// is fulfilled/blocked locally: never delivers Analytics data to a property.
...
try{
 for(const signedIn of [false,true]){
  ...
  results.push({signedIn,checks,packets,external,actualGoogleMeasurementRequests:0})
  console.log(JSON.stringify({signedIn,checks}))
  await context.close()
 }
}finally{
 await mkdir(output,{recursive:true})
 await writeFile(resolve(output,'real-tag-results.json'),JSON.stringify({tagSha256:source.sha256,results},null,2))
