...
test('dashboard cards use natural height and deliberate responsive placement', async () => {
  ...
  assert.match(styles, /\.business-dashboard__card--completion,[\s\S]*?\.business-dashboard__card--coverage \{[\s\S]*?grid-column: span 7;/)
  assert.match(styles, /\.business-dashboard__status-stack,[\s\S]*?\.business-dashboard__card--account-summary \{[\s\S]*?grid-column: span 5;/)
