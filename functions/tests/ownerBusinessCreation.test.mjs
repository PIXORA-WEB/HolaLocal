...
test('ensureOwnerBusiness creates one deterministic owner business and converges on retry', async () => {
  ...
  assert.equal(db.data('businesses/owner').subscription.schemaVersion, 1)
  assert.equal(db.data('businesses/owner').subscription.planId, 'early_access')
  assert.equal(db.data('businesses/owner').subscription.planRevision, 1)
  assert.equal(db.data('businesses/owner').subscription.accessStatus, 'active')
  assert.equal(db.data('businesses/owner').subscription.assignmentSource, 'system')
  assert.equal(db.data('businesses/owner').subscription.endsAt, null)
  assert.equal(db.data('businesses/owner').subscription.updatedBy, 'system')
  assert.ok(db.data('businesses/owner').subscription.assignedAt)
  assert.ok(db.data('businesses/owner').subscription.startsAt)
  assert.ok(db.data('businesses/owner').subscription.updatedAt)
