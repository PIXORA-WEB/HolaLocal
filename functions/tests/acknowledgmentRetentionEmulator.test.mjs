...
test('real transaction: closed gate, assessed release, reload, retry and terminal finalizer safety',{skip:!enabled},async()=>{
 ...
 const reloaded=(await ref.get()).data();assert.equal(reloaded.retainedConsentEvidence,undefined);assert.equal((await db.doc(`acknowledgmentRetention/${uid}`).get()).exists,false);assert.equal(reloaded.requestVersion,8);assert.equal(reloaded.lastCompletedStep,'completed')
...
test('completion creates due review once, with no backdating or duplicate resets',{skip:!enabled},async()=>{
 ...
 const first=(await db.doc(`acknowledgmentRetention/${uid}`).get()).data().decision;assert.equal(first.reviewerId,'admin');assert.equal(first.reviewAt.toMillis(),now.toMillis());assert.equal((await ref.get()).data().evidenceRetention,undefined)
 ...
 assert.deepEqual((await db.doc(`acknowledgmentRetention/${uid}`).get()).data().decision,first)
})
