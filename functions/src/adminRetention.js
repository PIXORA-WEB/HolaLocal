...
function validate(data) {
  ...
  if(data.action==='list'){if(data.cursor!=null&&!identifier(data.cursor))throw new HttpsError('invalid-argument','invalid-retention-cursor')}
...
export async function manageRetentionRecords({actorUid,claims,data,db,auth,env=process.env,now=Timestamp.now()}) {
  ...
  if(data.action==='list'){
    let query=db.collection(collections[data.kind]).orderBy(FieldPath.documentId())
