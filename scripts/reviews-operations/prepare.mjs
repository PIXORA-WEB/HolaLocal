...
export function prepare({projectId,notificationChannels=[]}={}) {
 ...
 const runtime=`resource.type="cloud_run_revision" AND resource.labels.project_id="${projectId}" AND resource.labels.location="europe-west1" AND (${selection} OR resource.labels.service_name="finalizeaccountdeletion")`
 const metrics=[
 ...
 function policy(id,condition,notes,log=false){return {displayName:`HolaLocal reviews: ${id}`,enabled:false,combiner:'OR',notificationChannels:['callable-server-errors','callable-latency'].includes(id)?[]:[...notificationChannels],documentation:{content:notes+' No review text, account IDs, tokens or private notes belong in incident labels. See docs/REVIEWS_OPERATIONS.md.',mimeType:'text/markdown'},conditions:[{displayName:id,...condition}],alertStrategy:{autoClose:'604800s',...(log?{notificationRateLimit:{period:'86400s'}}:{})},userLabels:{feature:'customer-reviews',managed_by:'reviewed-local-plan'}}}
