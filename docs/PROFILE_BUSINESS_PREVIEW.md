...
Baseline: main 9fd2023918ed56f3a8cc2a8ee469a6e2c9188a0f (PR #38). Branch: preview/profile-business-refresh. Appearance approved at 4786845c1fcd386256ba59d26502dbee4f1981e8. Release preparation and automatic PR preview authorised; production merge/deployment not authorised.
...
Subscription projection and insights show expected unavailability because this harness does not run Functions. Real photo upload transport is not retested; its existing implementation is unchanged. No new production, native-language editorial, full assistive-technology or exhaustive lifecycle visual verification is claimed. Appearance approved; production merge approval still required. Original mixed workspace, recovery backup and separate investigations preserved.
...
Current main reverified at 9fd2023918ed56f3a8cc2a8ee469a6e2c9188a0f; no integration changes required. Runtime source remains exactly the approved 4786845 source. Reuse its passing 25.1-second emulator browser journey, lint and fresh build; this release preparation only updates documentation. Production build inspection found no synthetic fixture email/name/owner/password markers or tests directories. Test fixtures remain under tests/browser, outside Vite's application entry graph; no test-only configuration changes are included. Durable screenshot gallery stays outside the repository. No production runtime fixtures added.

Scope: ProfilePage.jsx, BusinessDashboardPage.jsx, EditBusinessPage.jsx, original global.css, existing browser regression and this document. Functions, rules, indexes, environment settings, activation controls and dependencies are unchanged.

Release procedure after separate merge approval: merge the reviewed PR into main through the existing GitHub workflow; allow the existing Vercel project to build fresh from that merge. Verify deployment commit and production aliases, public/auth route guards, then owner Profile/Edit profile and My Business interactions. No Firebase deployment required. Rollback: restore the previous verified Vercel production deployment for main 9fd2023 (dpl_61p2WY6eBYeYi3YNgQmAyHpRg4yn), then prepare a normal source revert if needed. Preserve the known 200 kB budget failure; no limit or protection bypass authorised.
