...
See [EU release approval package](REVIEW_TRANSLATION_EU_RELEASE.md) for current verification, permission blockers and exact deployment scope. Earlier preflight/verification passages below are dated historical evidence, superseded where this package says otherwise. Both gates and provider selection stay disabled; no deployment is authorised by this document.
...
The authoritative review callable now explicitly passes translate-eu.googleapis.com and europe-west1 with a10-second deadline/no upstream retries through the existing provider factory. There is no review environment override to global. The Google cache namespace is google_cloud-eu-europe-west1-nmt-v2; previous global entries cannot be reused. Messaging defaults remain unchanged. Matching declared source/target returns original text without a provider call. Sanitized internal diagnostics separate RPC status from response-validation failures without content/credentials/raw metadata.

Real-provider evidence: authorised synthetic probe at bde093e returned16/16 nonempty EU translations,1,184 characters, estimated USD0.02368 before adjustments. Local ADC success is not runtime permission evidence or native-language editorial approval. The earlier en-to-en error remains unconfirmed. Translation API is now enabled by separate approval. Evidence is preserved outside Git in review-evidence/review-translation-eu/revised-run. [EU endpoints](https://docs.cloud.google.com/translate/docs/advanced/endpoints).
